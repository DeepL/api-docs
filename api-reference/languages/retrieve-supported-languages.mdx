---
openapi: get /v2/languages
title: "Retrieve supported languages (v2)"
tag: "DEPRECATED"
---

<Warning>
  **`/v2/languages` is deprecated.** Use [`GET /v3/languages`](/docs/languages/using-the-languages-api) instead. See the [migration guide](/docs/languages/migrating-from-v2-languages) for details.

  We do not yet have a timeline for sunsetting /v2/languages, and we'll share more detail on this topic by the end of Q2 2026.
</Warning>

