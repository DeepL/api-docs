...
- **Scoped** - the key has one or more permissions assigned and can only call the endpoints those permissions cover. Calls to other endpoints are rejected.
...
|:---|:---|
| Translate text | Text translation endpoints (`/v2/translate`) |
| View glossaries and their entries | Glossary read endpoints (`GET /v3/glossaries`, `GET /v3/glossaries/{glossary_id}`, `GET /v3/glossaries/{glossary_id}/entries`) |
| Create, modify, and delete glossaries | Glossary write endpoints (`POST`, `PATCH`, `PUT`, and `DELETE` on `/v3/glossaries`) |

Additional permissions covering more of the API will be added in future updates.
