...
<ParamField body="tag_handling" type="string">
  ...
  <ul>
    <li><code>xml</code>: Enable XML tag handling; see <a href="/docs/translate/translating-xml">Translating XML</a>.</li>
    <li><code>html</code>: Enable HTML tag handling; see <a href="/docs/translate/translating-html">Translating HTML</a>.</li>
  </ul>
...
<ParamField body="tag_handling_version" type="string">
  Select which version of the tag handling algorithm should be used.
  <ul>
    <li><code>v2</code>: Use the improved v2 algorithm (recommended). Sentences are translated more naturally without being constrained by tag placement in the source text, and edge cases around tag preservation, DOM hierarchy, and character escaping are handled better.</li>
    <li><code>v1</code>: Use the previous v1 algorithm.</li>
  </ul>
  Default: accounts that first used tag handling after December 1, 2025 default to <code>v2</code>; all other accounts default to <code>v1</code>. Both versions work with all <code>model_type</code>s.

  With <code>v2</code>, XML input is strictly parsed: invalid XML returns an error with the message <code>Tag handling parsing failed, please check input.</code> HTML input is not strictly parsed in either version.

  Results differ between v1 and v2, so test representative samples of your content (deep tag hierarchies, inline markup, attributes, special characters, empty and self-closing tags) before switching versions in production.
...
<ParamField body="non_splitting_tags" type="array[string]">
  Comma-separated list of XML tags which never split sentences. <a href="/docs/translate/translating-xml#keep-sentences-together-across-tags">Learn more</a>
</ParamField>
<ParamField body="splitting_tags" type="array[string]">
  Comma-separated list of XML tags which always cause splits. <a href="/docs/translate/translating-xml#control-sentence-splitting-manually">Learn more</a>
</ParamField>
<ParamField body="ignore_tags" type="array[string]">
  Comma-separated list of XML tags that indicate text not to be translated. <a href="/docs/translate/translating-xml#exclude-content-from-translation">Learn more</a>
</ParamField>
