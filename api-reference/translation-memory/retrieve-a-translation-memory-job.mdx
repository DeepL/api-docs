...
| `downloaded` | Export | The exported file has been fetched at least once. |
...
Poll until the status is `completed`, `downloaded`, `failed`, or `expired`. The other states are transient and will change on their own.
