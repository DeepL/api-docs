...
- Reports cover `docx`, `pptx`, `pdf`, `srt`, `idml`, `xml`, `dita`, `mif`, and XLIFF 2.1 uploads, and a subset of the language pairs translation supports. Each report also returns the source and translated text of every segment, plus the glossary entries that were applied to it.
