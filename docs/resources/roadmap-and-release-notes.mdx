...
- The Voice API now supports `fr-FR` (French, France) and `fr-CA` (French, Canadian) as target language variants for translation. `fr-FR` is generally available; `fr-CA` is in beta for the Voice API (for text and document translation it has been generally available since July 17).
