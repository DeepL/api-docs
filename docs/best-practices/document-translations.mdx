...
- The language pair must be in the [supported language pairs](/api-reference/quality-evaluation/poll#supported-language-pairs). Quality evaluation supports fewer language pairs than document translation.
...
Store the report as soon as it's `done`: reports are kept for [24 hours](/api-reference/quality-evaluation/poll#report-retention) after the evaluation finishes, and polling after that returns HTTP 404. For the severity and issue-type vocabulary, how to resolve `source_spans` and `target_spans`, and the applied glossary term pairs a report returns per segment, see the [quality evaluation reference](/api-reference/quality-evaluation/poll).
