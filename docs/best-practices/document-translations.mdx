...
We impose [size limits](/docs/resources/usage-limits) per file type. Limits vary by format — for example, `.docx` / `.pptx` / `.pdf` can be up to 100 MB on API Pro, `.idml` / `.mif` / `.xlsx` up to 30 MB, `.json` 1 MB, `.dita` 5 MB.
