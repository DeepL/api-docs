---
title: "Write API"
description: "Improve text with the DeepL Write API: fix spelling and grammar, rewrite for clarity, and control writing style and tone."
covers: [Write]
public: true
---
<Info>
  DeepL API for Write is generally available via the v2 API endpoint for customers with a DeepL API Pro subscription.

  The existing provisions applying to customers' DeepL API Pro subscription also apply to DeepL API for Write with [the following additions to the Service Specification](/api-reference/improve-text/deepl-write-api-service-specification-updates). [An FAQ](#frequently-asked-questions) is included at the bottom of this page.
</Info>
...
For more detail about request body parameters, see the [Request body descriptions](#request-body-descriptions) section further down the page.
...
<ParamField body="target_lang" type="string">
  ...
  * `zh`/`zh-Hans` (simplified Chinese)
  ...
  You can also retrieve supported languages programmatically via <a href="/docs/languages/using-the-languages-api"><code>GET /v3/languages?resource=write</code></a>.
...
<ParamField body="writing_style" type="enum">
  ...
  * `pt-PT` (Portuguese)
  ...
  To check writing style support dynamically, call <a href="/docs/languages/using-the-languages-api"><code>GET /v3/languages?resource=write</code></a> and look for the <code>writing_style</code> feature key on the target language.
</ParamField>
<ParamField body="tone" type="enum">
  ...
  * `pt-PT` (Portuguese)
  ...
  It’s not possible to include both `writing_style` and `tone` in a request; only one or the other can be included. To check tone support dynamically, call <a href="/docs/languages/using-the-languages-api"><code>GET /v3/languages?resource=write</code></a> and look for the <code>tone</code> feature key on the target language.
</ParamField>
...
Yes, the `character_count` field in the `/usage` endpoint response returns a sum of Translate API + Write API characters. You can learn more about the `/usage` endpoint [here](/api-reference/usage-and-quota/check-usage-and-limits).
...
We've added a new column to the [API key-level usage report](/docs/admin/retrieving-usage-data#api-key-level-csv-export) that breaks out text improvement characters separately. The character counts in the [Usage tab](https://www.deepl.com/your-account/usage) of your API account are a sum of both Translate *and* Write characters; these are not yet broken out separately.
