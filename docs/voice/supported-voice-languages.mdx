...
Parenthesized regional variants (e.g. `en-US`, `pt-BR`, `fr-CA`) are additional translation target options; the other columns describe the base language. A beta badge inside the parentheses applies to that variant only — `fr-CA` is beta as a translation target.
</Card>
