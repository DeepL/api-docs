...
| Estonian | ⎋ | ✓ | — |
| Finnish | ⎋ | ✓ | ⎋ |
| French (France/Canada <Badge color="blue">beta</Badge>) | ✓ | ✓ | ✓/⎋ |
| German | ✓ | ✓ | ✓ |
| Greek | ⎋ | ✓ | ⎋ |
| Hebrew | ⎋ | ✓ | — |
...
Parenthesized regional variants (e.g. `en-US`, `pt-BR`, `fr-CA`) are additional target-language options (translation, and translated speech where the base language has it); the other columns describe the base language. A beta badge inside the parentheses applies to that variant only — `fr-CA` is beta as a translation target.
</Card>
