...
By default, each target language is spoken by its own language-specific voice, and you can instead request a preset male or female voice on the [session request](/api-reference/voice/request-session). **Speaker match** is a third option: it produces translated speech with a voice close to the original speaker's voice. Speaker match works only for target languages whose translated speech is provided by DeepL, not for languages provided through external service partners. See [Supported Voice Languages](/docs/voice/supported-voice-languages) for which languages qualify. Requesting speaker match for any other target language is not an error: the session starts as usual and that language is spoken by a preset voice instead, so check the language table rather than relying on a failed request.
